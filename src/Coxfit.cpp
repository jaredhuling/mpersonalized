...
extern "C" {
  ...
  void linSolver(double *X, double *y, int* index, int *nrow, int *ncol,
                 ...
                 double *step, int *reset, double *weights, double *penaltyweights)
  {
    double *theta = new double[ncol[0]];
    int startInd = 0;
    double zeroCheck = 0;
    ...
    for(int i = 0; i < numGroup[0]; i++)
    {
      if(useGroup[i] == 1)
      {
        ...
        for(int j = 0; j < groupLen[i]; j++)
        {
          ...
          int varidx = j + rangeGroupInd[i];
          double lam1 = lambda1[0] * penaltyweights[varidx];

          if(grad[j] < lam1 && grad[j] > -lam1)
          {
            grad[j] = 0;
          }
          if(grad[j] > lam1)
          ...
          while(count <= innerIter[0] && check > thresh[0])
          {
            ...
            while(diff < 0)
            {
              for(int j = 0; j < groupLen[i]; j++)
              {
                ...
                z[j] = beta[varidx] - t * grad[j];

                if(z[j] < lam1 * t && z[j] > -lam1 * t)
                {
                  z[j] = 0;
                }
                if(z[j] > lam1 * t)
  ...
  int linNest(double *X, double* y, int* index, int *nrow, int *ncol, int *numGroup,
              ...
              double *eta, double *gamma, int *betaIsZero, double *step, int *reset, double *weights, double *penaltyweights)
  {
    double* prob = NULL;
    ...
    while(groupChange == 1)
    {
      ...
      linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                ...
                useGroup, step, reset, weights, penaltyweights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        linSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                  groupLen, lambda1, lambda2, innerIter, thresh, ldot,
                  nullBeta, gamma, eta, betaIsZero, groupChange, isActive,
                  tempIsActive, step, reset, weights, penaltyweights);
  ...
  void logitSolver(double *X, int *y, int* index, int *nrow, int *ncol, int *numGroup,
                   ...
                   double *betaZero, double *step, double *weights, double *penaltyweights)
  {
    double *theta = new double[ncol[0]];
    double *thetaNew = new double[ncol[0]];
    int startInd = 0;
                ...
                double *penaltyweights)
  {
    ...
    while(groupChange == 1)
    {
      ...
      logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                  ...
                  prob, betaZero, step, weights, penaltyweights);
      ...
      while(outermostCounter < outerIter[0] && outermostCheck > outerThresh[0])
      {
        ...
        logitSolver(X, y, index, nrow, ncol, numGroup, beta, rangeGroupInd,
                    ...
                    isActive, prob, betaZero, step, weights, penaltyweights);
