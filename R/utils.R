...
#' @useDynLib mpersonalized

betterPathCalc.mod <- function (data, index, alpha = 0.95, min.frac = 0.05, nlam = 20,
          type = "linear")
{
  reset <- 10
  step <- 1
...
oneDim.mod <- function (data, index, thresh = 1e-04, nlam = 20, lambdas = NULL,
...
{
  if (is.null(lambdas)) {
    lambdas <- betterPathCalc.mod(data = data, index = index,
  ...
  for (k in 1:nlam) {
    ...
    junk <- .C("linNest", X = as.double(as.vector(X)), y = as.double(y),
               ...
               step = as.double(step), reset = as.integer(reset), PACKAGE = "SGL")
    beta.new <- junk$beta
    beta[, k] <- beta.new
...
oneDimLogit.mod <- function (data, index, thresh = 1e-04, lambdas = NULL, beta.naught = rep(0, ncol(data$x)),
                             inner.iter = 100, outer.iter = 100, outer.thresh = 1e-04,
                             gamma = 0.8, step = 1, reset = 10, alpha = 0.95, min.frac = 0.05,
                             nlam = 20, verbose = FALSE)
{
  if (is.null(lambdas)) {
    lambdas <- betterPathCalc.mod(data = data, index = index,
                                  alpha = alpha, min.frac = min.frac, nlam = nlam,
  ...
  for (i in 1:nlam) {
    junk <- .C("logitNest", X = as.double(as.vector(X)),
               ...
               ncol = as.integer(ncol(X)), numGroup = as.integer(num.groups),
               rangeGroupInd = as.integer(range.group.ind), groupLen = as.integer(group.length),
               lambda1 = as.double(alpha * lambdas[i]), lambda2 = as.double((1 - alpha) * lambdas[i]),
               ...
               betaZero = as.double(intercepts[i]), step = as.double(step), PACKAGE = "SGL")
...
SGL.mod <- function (data, index, type = "linear", maxit = 1000, thresh = 0.001,
                     ...
                     verbose = FALSE, step = 1, reset = 10, alpha = 0.95, lambdas = NULL)
{
  ...
  if (type == "linear") {
    ...
    Sol <- oneDim.mod(data, index, thresh, inner.iter = maxit,
  ...
  if (type == "logit") {
    Sol <- oneDimLogit.mod(data, index, thresh = thresh, inner.iter = maxit,
                       outer.iter = maxit, outer.thresh = thresh, min.frac = min.frac,
                       nlam = nlam, lambdas = lambdas, gamma = gamma, verbose = verbose,
  ...
  if (type == "cox") {
    Sol <- SGL:::oneDimCox(data, index, thresh = thresh, inner.iter = maxit,
                     outer.iter = maxit, outer.thresh = thresh, min.frac = min.frac,
                     nlam = nlam, lambdas = lambdas, gamma = gamma, verbose = verbose,
                     step = step, alpha = alpha, reset = reset)
    Sol = list(beta = Sol$beta, lambdas = Sol$lambdas, type = type,
...
SGL2 <- function (data, index, type = "linear", maxit = 1000, thresh = 0.001,
                  min.frac = 0.1, nlam = 20, gamma = 0.75, standardize = TRUE,
                  verbose = FALSE, step = 1, reset = 10, alpha = 0.95, lambdas = NULL,
                  weights = rep(1, NROW(data$x)), penaltyweights = rep(1, NCOL(data$x)))
{
...
oneDimLogit2 <- function (data, index, thresh = 1e-04, lambdas = NULL, beta.naught = rep(0, ncol(data$x)),
                          inner.iter = 100, outer.iter = 100, outer.thresh = 1e-04,
                          gamma = 0.8, step = 1, reset = 10, alpha = 0.95, min.frac = 0.05,
                          nlam = 20, verbose = FALSE, groupW,
                          weights = rep(1, NROW(data$x)), penaltyweights = rep(1, NCOL(data$x)))
{
  if (is.null(lambdas)) {
  ...
  for (i in 1:nlam) {
    ...
    junk <- .C("logitNest",
               ...
               lambda1 = as.double(alpha * lambdas[i]),
               lambda2 = as.double((1 - alpha) * lambdas[i]),
               beta = as.double(beta.old),
               ...
               betaZero = as.double(intercepts[i]),
               step = as.double(step),
...
betterPathCalc2 <- function (data, index, alpha = 0.95, min.frac = 0.05, nlam = 20,
                             type = "linear", penaltyweights = rep(1, NCOL(data$x)))
{

  inv_penweight <- 1/penaltyweights
  inv_penweight[penaltyweights == 0] <- 0
  ...
  if (type == "cox") {
    ...
    junk1 <- .C("Cox", riskSetInd = as.integer(risk.set.ind),
                riskSet = as.integer(risk.set), numDeath = as.integer(num.deaths),
                status = as.integer(ordered.status), ndeath = as.integer(length(death.times)),
                nrow = as.integer(n), ncol = as.integer(p), beta = as.double(rep(0, p)),
                eta = as.double(rep(0, n)), y = as.double(rep(0, n)), weights = as.double(rep(0, n)))
    resp <- junk1$y * junk1$weights
  }
  lambda.max <- rep(0, num.groups)
  if ((alpha != 0) * (alpha != 1)) {
    for (i in 1:num.groups) {
      ind <- groups[i]
      X.fit <- X[, which(index == ind)]
      cors <- inv_penweight[which(index == ind)] * (t(X.fit) %*% resp)
      ord.cors <- sort(abs(cors), decreasing = TRUE)
      if (length(ord.cors) > 1) {
        norms <- rep(0, length(cors) - 1)
        lam <- ord.cors/alpha
  ...
  if (alpha == 1) {
    lambda.max <- inv_penweight * (abs(t(X) %*% resp))
  }
  if (alpha == 0) {
    for (i in 1:num.groups) {
      ind <- groups[i]
