...
When `unique_rule = TRUE`, the `penalty` function only has two choice: `none` and `lasso`, corresponding to the case $\lambda_{unique} = 0$ and $\lambda_{unique} \ne 0$.

`unique_rule_lambda`:

This argument corresponds to $\lambda_{unique}$. It could be leave null to use the default sequence or supplied to use a user specified sequence.
...
If `unique_rule = TRUE`, then `unique_ind` should be supplied instead of `ind1` and `ind2`.
